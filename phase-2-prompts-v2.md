...
## Revision notes (v2 — reviewed against the master plan and the DSL)

This file was reviewed against `docs/plan.md` and `packages/dsl/src/schemas.ts`. The structure and prompt order are unchanged. Five targeted amendments — read these as part of the prompts they touch:

1. **(2.1) Relation id format.** Deterministic ids must satisfy the DSL's ULID regex (`RelationDef.id`, and Phase 3's `JoinDef.via`). A raw hex hash fails validation. Encode the hash as 26 chars of Crockford base32 instead — spec'd in 2.1.
2. **(2.3) m:n becomes a real RelationDef, not a synthetic id.** The DSL has a first-class many-to-many shape with a `junction` field; Phase 3's structured joins reference relations by id. Junction detection now emits genuine m:n RelationDefs; the synthetic `junction:<id>` namespace is dropped.
3. **(2.3) Manual junction override added** (`auto | always | never` per table) — conservative auto-detection needs an escape hatch on real schemas.
4. **(2.5) Compound-PK batch labels need a decision:** single-column `IN` can't express tuple lookups; the prompt now mandates choosing tuple-IN adapter support or OR-of-ANDs compilation.
5. **(2.7) The dangling "full breadcrumb UI lands in 2.7" reference is resolved** — a small prompt 2.7 now exists (persistence, overflow, keyboard back-step).

Conscious scope cut, stated rather than implied: the custom-relation editor (2.4) supports 1:1 and 1:n only; user-defined m:n with a custom junction is deferred (revisit alongside Phase 3 joins).
...
- A new module that, given a SchemaSnapshot, derives RelationDefs from every
  foreign key including compound and self-referential. The derivation function
  is PURE (snapshot in, RelationDefs out — no DB access) so edge cases can be
  unit-tested with hand-built snapshot fixtures in addition to the
  testcontainers integration tests below.
  Stable id: a deterministic hash of (source schema, source table, source
  columns, target schema, target table, target columns) so the same FK
  produces the same RelationDef id across reconnects. ID FORMAT MATTERS: the
  DSL validates RelationDef.id (and Phase 3's JoinDef.via) against the ULID
  regex ^[0-9A-HJKMNP-TV-Z]{26}$. Encode the hash (e.g. SHA-256 truncated to
  130 bits) as 26 characters of Crockford base32 so it passes that regex.
  A raw hex digest will be REJECTED by the validators. Add a unit test that
  every derived id round-trips through validateRelation. `source: "schema"`
  for these.
...
- detectJunctions(connectionId) -> Map<TableKey, JunctionInfo>
  ...
  junction couples.
- MANUAL OVERRIDE: a per-table junction policy `auto | always | never`,
  persisted in the metadata store (same (host, port, database) scope as
  custom relations; a small KV or a displayConfig extension — pick what
  fits). `always` forces junction treatment even if the heuristic
  disqualifies; `never` suppresses it even if the heuristic matches.
  detectJunctions consults the policy. Surface the toggle in the
  per-table settings popover (the gear from 2.5 — wire the UI there once
  2.5 lands; the engine support belongs in this prompt).
- For each detected junction, EMIT A REAL m:n RelationDef: cardinality
  "many-to-many", the `junction` field populated with the junction table
  and its column pairs, source: "schema", and a deterministic id in the
  same Crockford-base32 format as 2.1. These m:n RelationDefs are merged
  into listRelations like every other relation — Phase 3's structured
  joins reference relations by id, so m:n must exist as first-class
  RelationDefs, not as a synthetic id namespace. Do NOT invent
  `junction:<id>` strings.
- getReferencingCounts(connectionId, schema, table, pkValues)
  ...
  For each m:n relation the (visible) source table participates in,
  return an entry under that relation's REAL id with the collapsed
  two-hop count (e.g., a customer's distinct tag count via
  customer_tags).
...
- The emitted m:n RelationDef validates through validateRelation
  (cardinality many-to-many, junction populated) and appears in
  listRelations with a stable id across two introspections.
- Override `never` on customer_tags removes the m:n relation and
  surfaces customer_tags as an ordinary table; override `always` on a
  heuristic near-miss forces junction treatment. Policy round-trips
  through the metadata store.
- getReferencingCounts for a specific customer returns the right number
  of orders + the collapsed tag count via the junction (keyed by the
  real m:n relation id).
```
...
- Create-relation form (shadcn dialog):
    ...
    * Cardinality: one-to-many | one-to-one
      (DELIBERATE SCOPE CUT: no user-defined many-to-many with a custom
      junction in this phase. Every 1:n is expressible with the unique
      end as the target; custom m:n returns alongside Phase 3's joins.)
...
- getRowLabels(connectionId, schema, table, pkTuples[]) -> string[]
  ...
  COMPOUND-PK DECISION REQUIRED: the FilterOp "in" is single-column and
  cannot express a tuple lookup (warehouses' PK is (tenant_id, code)).
  Pick ONE and document it in the adapter README:
    (a) extend the adapter's filter compiler with a tuple-IN capability
        compiling to `(a, b) IN ((...), (...))`, or
    (b) compile the batch as an OR of per-tuple AND groups through the
        existing FilterGroup machinery.
  Either is fine at batch sizes ≤ 200; (b) needs no adapter change but
  produces longer SQL. Whichever you choose, the test below asserts a
  single round trip for a compound-PK batch.
...
- getRowLabels batches a single SQL round trip; verified by adapter test
  ...
  separate queries.
- getRowLabels with COMPOUND PK tuples (warehouses) also resolves in a
  single round trip via whichever mechanism was chosen, and returns
...
## Prompt 2.7 — Breadcrumb completion: persistence, overflow, back-step

```
Finish the breadcrumb trail whose foundation landed in 2.2.

- Persistence: filteredTable tabs already carry their breadcrumb array in
  the OpenTab union; ensure the tabs-storage Zod schema round-trips it so
  multi-hop trails survive quit + relaunch with the rest of the tab state.
  (BreadcrumbStep filters are FilterGroups — reuse the DSL schema, don't
  redefine the shape.)
- Overflow: long trails (5+ hops) collapse the middle into an ellipsis
  dropdown — head crumb, "…", last two crumbs. The dropdown lists the
  collapsed steps, each clickable.
- Back-step: Cmd/Ctrl+[ (and a back arrow at the trail's head) re-opens
  the previous step, same behavior as clicking the second-to-last crumb.
- Self-referential honesty: a manager->manager->manager chain renders
  every hop, no deduplication (per the failure-mode note) — the overflow
  dropdown is what keeps this usable.
- Labels in crumbs use formatRowLabel (2.5) and update if a DisplayConfig
  changes; falling back to PK values when unconfigured.

Tests:
- tabs-storage round-trip of a 4-hop filteredTable tab.
- A renderer test for the overflow collapse boundary (4 hops inline,
  5 hops collapses).
```

**Verify:** Jump customer → orders → order → items → product (5 hops); the trail collapses the middle into "…"; relaunch the app — the tab and its full trail restore; Cmd/Ctrl+[ steps back one hop.
...
- Junction-table detection correctly identifies `customer_tags` and ignores `order_items`; detected m:n relations exist as first-class RelationDefs (validating through the DSL) and the per-table `auto | always | never` override works and persists.
...
- Every relation id — schema-derived, m:n, custom — passes the DSL's ULID-format validation (Phase 3's JoinDef.via depends on this).
- DisplayConfig changes FK labels and breadcrumbs from "42" to "Ada Lovelace (FR)".
- Multi-hop breadcrumb trails persist across relaunch, collapse gracefully when long, and support keyboard back-step.
...
- **Relation ids that fail the DSL's own validation.** The ULID regex on `RelationDef.id` rejects hex digests. Every derived id (schema FKs and m:n alike) must be 26 chars of Crockford base32, and the round-trip-through-validateRelation test must exist — otherwise the failure surfaces in Phase 3 when `JoinDef.via` validation rejects ids that have been in the metadata store for weeks.
